...
const cardManagerFile = path.join(__dirname, "../card-manager/src/types/shared.ts");
...
fs.watchFile(sourceFile, (curr, prev) => {
    ...
    fs.copyFile(sourceFile, cardManagerFile, (err) => {
        if (err) throw err;
        console.log(`Copied to: ${cardManagerFile}`);
    });
});