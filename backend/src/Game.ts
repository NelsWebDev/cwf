...
import { BlackCard, CardDeck, CardState, DEFAULT_RULES, Rules, WhiteCard, Game as TGame } from "./types";
...
export class Game {
    ...
    async start() {
        ...
        const minWhiteCards = (this.players.length * this.rules.pointsToWin ) -1 + (this.players.length * 10);
        ...
        this._whiteCards.push(...CardManager.makeBlankWhiteCards(this.rules.numberOfCustomCards));
    ...
    winningPlayer() {
        ...
        const winner = Array.from(this._points.entries()).find(([userId, points]) => points === highScore);
        if (!winner) {
            return undefined;
        }
    ...
    shuffleBlackCards() {
        const cardsToReset = this._blackCards.filter(card => card.state === CardState.AVAILABLE || card.state === CardState.PLAYED_PREVIOUSLY);
        cardsToReset.forEach(card => card.state = CardState.AVAILABLE);
        this._blackCards = this._blackCards.sort(() => Math.random() - 0.5);
    }

    shuffleWhiteCards() {
        const cardsToReset = this._whiteCards.filter(card => card.state === CardState.AVAILABLE || (card.state === CardState.PLAYED_PREVIOUSLY && !card.isCustom));
        cardsToReset.forEach(card => card.state = CardState.AVAILABLE);
    ...
    private async fetchCards() {
        ...
        const [blackCards, whiteCards] = await Promise.all([
            prismaClient.blackCard.findMany({
                where: {
                    deckId: {
                        in: addedDeckIds,
                    },
            ...
            }).then(cards => cards.map<BlackCard>(card => ({
                ...card,
                state: CardState.AVAILABLE,
            }))),
            prismaClient.whiteCard.findMany({
                where: {
                    deckId: {
                        in: addedDeckIds,
                    }
                },
            }).then(cards => cards.map<WhiteCard>(card => ({
                ...card,
                state: CardState.AVAILABLE,
                isCustom: false,
            })))
    ...
    getNextCzar() {
        ...
        if(!this._currentCzar) {
            const randomId = Math.floor(Math.random() * socketManager.activeUsers.length);
            return socketManager.activeUsers[randomId];
        }

        const indexOfCurrentCzar = socketManager.activeUsers.findIndex(user => user.id === this._currentCzar?.id);
        if(indexOfCurrentCzar === -1) {
            return socketManager.activeUsers[0];
        }

        const nextCzarIndex = (indexOfCurrentCzar + 1) % socketManager.activeUsers.length;
    ...
    drawBlackCard() {
        const card = this._blackCards.find(card => card.state === CardState.AVAILABLE);
    ...
    get availableWhiteCards() {
        return [...this._whiteCards.filter(card => card.state === CardState.AVAILABLE)];
    }
