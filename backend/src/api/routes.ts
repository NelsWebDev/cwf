...
routes.post("/login", async ({body}, res) => { 
    try {
        ...
        if("id" in body) {
            ...
            if(!user) {
                res.status(400).json({ success: false, error: "Session expired. Please login again" });
                return;
            }
            res.json(user);
            return;
...
routes.post("/decks/import", async (req, res) => {
    ...
    try {
       ...
       res.json({deck});
        ...
        const message = error instanceof Error  && error.message === "Deck already imported" ? "Deck already imported" : "Failed to import deck";
